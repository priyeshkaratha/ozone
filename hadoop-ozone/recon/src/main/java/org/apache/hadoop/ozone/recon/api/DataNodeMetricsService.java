...
public class DataNodeMetricsService {
        ...
        .handle((result, throwable) -> {
          ...
          for (CompletableFuture<DatanodePendingDeletionMetrics> future : futures) {
            try {
              ...
              DatanodePendingDeletionMetrics metrics = future.getNow(null);
              if (metrics != null) {
        ...
        .thenApply(metrics -> {
          try {
            ...
            long pendingBlockSize = ReconUtils.extractMetricValue(
                deletionMetrics, "TotalPendingBlockBytes");
  ...
  private static class MetricsState {
    ...
    MetricsState() {
      this(MetricCollectionStatus.NOT_STARTED, new ArrayList<>(), 0L, 0, 0, 0);
    }
